...
@ray.remote
def quad(d, fun, x, basis, sigma, gh_roots, gh_weights):
    # estimate directional derivative via Gauss--Hermite quadrature
    f_d_vals = np.array([fun(x + sigma * p * basis[d]) for p in gh_roots])
    return np.sum(gh_weights * gh_roots * f_d_vals) / (sigma * np.sqrt(np.pi)/2)
...
def DGS(fun, x0, args=(), learning_rate=.01, sigma=.1, quad_points=7,
    ...
    def step(x):
        ...
        df_sigma_basis = np.array(ray.get([
            quad.remote(d, fun, x, basis, sigma, gh_roots, gh_weights)
            for d in range(dim)]))
...
if __name__ == '__main__':
    ...
    ##fun = lambda x: sum([x[d]**2 for d in range(100)])
    np.random.seed(0)
    ...
    print(res)
    ray.shutdown()
    print(f'\ntime elapsed: {(time.time() - t0) / 60:.2f} min')
